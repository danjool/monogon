...
function init() 
{
	...
	container = document.getElementById( 'raytrace2' );
	...
	var rayCastMaterial = new THREE.ShaderMaterial({
		...
		fragmentShader: document.getElementById('rayCastFragFaceCamera').textContent,
...
for (i = mn; i <= mx; i++){
	for (j = mn; j<= mx; j++){
		object = new THREE.ArrowHelper( new THREE.Vector3( j*50/arrdensity, -fakeCamPos.y+(50/arrdensity)*i+25, -fakeCamPos.z+25 ).normalize(), new THREE.Vector3( 0, 0, 0 ), 100, 0xF92672, 1,.1 );
...
var geometry = new THREE.CylinderGeometry( 10, 1, 10, 4, 1, true);
...
function update()
{
	...
	for (var c in scene.children) {
		if (scene.children[c].name == "arrow"){
			...
			if (l != 200) scene.children[c].setColor(0xF92672)
			else scene.children[c].setColor(0x3E3D32)
