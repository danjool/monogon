...
function init() 
{
	...
	container = document.getElementById('canvasiso');

	if ( Detector.webgl )
		renderer = new THREE.WebGLRenderer( {antialias:false, canvas: container} );
	else
		renderer = new THREE.CanvasRenderer(); 
	renderer.setSize(SCREEN_WIDTH/2.0, SCREEN_HEIGHT/2.0, false);
	// renderer.domElement.width = SCREEN_WIDTH+"px";
	// renderer.domElement.height = SCREEN_HEIGHT+"px";
	renderer.setClearColor( 0x272822, 1 );

	//container = document.getElementById( 'isosurface' );
...
"void main(){ vUv = uv; myPos = modelMatrix * vec4( position, 1.0);	myVert = position;"+
	...
	var rayCastMaterial = new THREE.ShaderMaterial({
		...
		fragmentShader: document.getElementById('rayCastFragIsoSurface2').textContent,
	...
	rayCastPlane.position.set(0, 0, 25);
