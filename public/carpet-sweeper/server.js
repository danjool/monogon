...
// Security tracking (moved before first use)
...
function rateLimitMiddleware(req, res, next) {
  ...
  const requestData = ipRequests.get(ip) || { count: 0, windowStart: now };
...
// Serve static files (client code) - serve from current directory
...
// Connection limit middleware for Socket.IO
const MAX_PLAYERS = process.env.MAX_PLAYERS || 4; // Default 4 players per server
...
io.use((socket, next) => {
  ...
  // Still track per-IP for rate limiting, but allow more connections per IP
...
io.on('connection', (socket) => {
  ...
  socket.on('disconnect', () => {
    ...
    if (player) {
      console.log(`Player ${player.name} disconnected (IP: ${ip})`);
  ...
  socket.on('join_game', (playerData) => {
    ...
    const ip = socket.handshake.address;
    console.log(`Player ${player.name} joined the game (IP: ${ip})`);
  });
  ...
  socket.on('mine_click', (data) => {
    ...
    if (cell && cell.state === 'hidden') {
      ...
      if (result.gameStatus !== 'playing') {
        console.log(`Game ${result.gameStatus}! Resetting in 10 seconds...`);
...
server.listen(PORT, '0.0.0.0', () => {  // '0.0.0.0' = all network interfaces
  console.log(`Flying Carpet Multiplayer Server running on port ${PORT}`);
